export interface DesignQualityBrowserObservation{
  ...
  computed:{viewport:{width:number;height:number};h1Count:number;h2Count:number;h1Px:number;medianH2Px:number;fontFamilies:string[];sectionCount:number;sectionHeights:number[];renderedColumns:number[];layouts:string[];distinctSectionBackgrounds:number;spacingGapMean:number;spacingGapStdDev:number;pageWidth:number;pageHeight:number;overflowX:boolean;ctaSectionCount:number;actionTargets:Array<{width:number;height:number;visible:boolean}>;mediaStages:number;motionStates:string[];contentBudgetPass:boolean;cssTokens:Record<string,string>};
