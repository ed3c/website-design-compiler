...
export interface PageGraphAuthoringData {
  ...
  root:{props:{project:string;category:string;route:string;source:CompletePageGraph["source"];readiness:CompletePageGraph["readiness"];semanticOrder:string[];conversionPath:string[];sharedChrome:CompletePageGraph["sharedChrome"];contracts:CompletePageGraph["contracts"];signature:string;missingEvidence:string[]}};
}
...
export function pageGraphToPuck(graph:CompletePageGraph):PageGraphAuthoringData{
  return{schema:"website-design-compiler/puck-page-graph/v2",content:graph.nodes.map((node)=>({type:"GovernedPageSection",props:structuredClone(node)})),root:{props:{project:graph.project,category:graph.category,route:graph.route,source:structuredClone(graph.source),readiness:graph.readiness,semanticOrder:[...graph.semanticOrder],conversionPath:[...graph.conversionPath],sharedChrome:structuredClone(graph.sharedChrome),contracts:structuredClone(graph.contracts),signature:graph.signature,missingEvidence:[...graph.missingEvidence]}}};
}
export function puckToPageGraph(data:PageGraphAuthoringData):CompletePageGraph{
  ...
  return{schema:"website-design-compiler/page-graph/v2",project:root.project,category:root.category,route:root.route,source:structuredClone(root.source),readiness:root.readiness,missingEvidence:[...root.missingEvidence],semanticOrder:[...root.semanticOrder],conversionPath:[...root.conversionPath],nodes,sharedChrome:structuredClone(root.sharedChrome),contracts:structuredClone(root.contracts),signature:root.signature};
}
export function puckToPayload(data:PageGraphAuthoringData):PayloadPageGraphDocument{
  ...
  return{schema:"website-design-compiler/payload-page-graph/v2",project:root.project,category:root.category,route:root.route,source:structuredClone(root.source),readiness:root.readiness,semanticOrder:[...root.semanticOrder],conversionPath:[...root.conversionPath],sharedChrome:structuredClone(root.sharedChrome),contracts:structuredClone(root.contracts),signature:root.signature,missingEvidence:[...root.missingEvidence],layout:graph.nodes.map((node)=>({blockType:"governed-page-section",...structuredClone(node)}))};
}
