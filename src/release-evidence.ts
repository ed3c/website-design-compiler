...
async function validateProductionProviderArtifacts(root:string,receipt:JsonRecord,expectedGit?:{sha:string;ref:string;tree?:string}):Promise<string[]>{
  ...
  const executedAsset=isRecord(executed.asset)?executed.asset:null;const provenance=isRecord(executed.provenance)?executed.provenance:null;const assetValidation=isRecord(requestBinding.assetValidation)?requestBinding.assetValidation:null;
  if(!executedAsset||!provenance||!assetValidation){errors.push("production execution receipt lacks asset or provenance evidence");return errors;}
  ...
  if(executedAsset.sha256!==sha256Value(assetBytes)||executedAsset.sha256!==receipt.assetSha256||executedAsset.bytes!==assetBytes.byteLength||assetBytes.byteLength>Number(assetValidation.maxBytes))errors.push("production execution receipt does not bind the persisted asset bytes and budget");
  try{
    const validationParameters:Record<string,number>={};if(typeof assetValidation.width==="number")validationParameters.width=assetValidation.width;if(typeof assetValidation.height==="number")validationParameters.height=assetValidation.height;
    const content=validateProductionMediaAssetContent(String(requestBinding.kind) as never,{bytes:assetBytes,mediaType:String(executedAsset.mediaType),extension:String(executedAsset.extension)},validationParameters);
  ...
  const observedSeedSha256=provenance.seed===null?"ABSENT":sha256Value(canonicalMediaValue(provenance.seed));
  if(assetValidation.seedSha256!==observedSeedSha256)errors.push("production provenance seed does not match the redacted request binding");
  return errors;
}
