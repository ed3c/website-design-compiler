...
export interface RightsSubject {
  id: string;
  kind: "package" | "asset" | "font" | "model" | "service";
  name: string;
...
export interface RepositoryClearanceReceipt {
  schema: "website-design-compiler/repository-rights-clearance/v2";
  overall: "PASS" | "FAIL";
  generatedAt: string;
  subjects: RightsSubject[];
  counts: Record<RightsState, number>;
  unresolved: string[];
  expiredWaivers: string[];
  noticeSubjects: string[];
  legalDisclaimer: "ENGINEERING_CLEARANCE_NOT_LEGAL_ADVICE";
}

const PERMISSIVE = ["MIT", "ISC", "Apache-2.0", "BSD-2-Clause", "BSD-3-Clause", "0BSD", "Unlicense", "CC0-1.0", "BlueOak-1.0.0"];
...
export function classifyLicense(expression: string | null): RightsState {
  ...
  if (PERMISSIVE.includes(expression.trim())) return "ALLOW";
  return "UNKNOWN";
}

interface PnpmDependency { name?: string; version?: string; dependencies?: Record<string, PnpmDependency>; optionalDependencies?: Record<string, PnpmDependency>; }
...
async function indexPackage(path: string, expectedName: string, index: Map<string, { license: string | null; path: string }>, root: string): Promise<void> {
  ...
  } catch { /* optional package without metadata */ }
}
...
export function applyWaivers(subjects: RightsSubject[], waivers: Waiver[], now: Date): { subjects: RightsSubject[]; expiredWaivers: string[] } {
  ...
  return {
    subjects: subjects.map((subject) => {
      const waiver = bySubject.get(subject.id);
      if (!waiver) return subject;
...
export async function scanRepositoryRights(root = process.cwd(), waivers: Waiver[] = [], now = new Date()): Promise<RepositoryClearanceReceipt> {
  ...
  const projects = JSON.parse(stdout) as PnpmProject[];
  const dependencies = flattenTree(projects);
  ...
  const packageSubjects: RightsSubject[] = [...dependencies.values()].sort((a, b) => `${a.name}@${a.version}`.localeCompare(`${b.name}@${b.version}`)).map(({ name, version }) => {
    const evidence = metadata.get(`${name}@${version}`);
    const license = evidence?.license ?? null;
    const state = classifyLicense(license);
    return { id: `package:${name}@${version}`, kind: "package" as const, name, versionOrIdentity: version, licenseExpression: license, state, evidence: evidence ? [evidence.path, `pnpm production graph:${name}@${version}`] : [`pnpm production graph:${name}@${version}`], attributionRequired: state === "ALLOW", distributed: true };
  });
  const staticAssets = await shippedAssets(root);
  const fixedSubjects: RightsSubject[] = [
    { id: "font:system-stack", kind: "font", name: "system font stack", versionOrIdentity: "runtime-system", licenseExpression: null, state: "NOT_DISTRIBUTED", evidence: ["apps/site/app/theme.generated.css or semantic token projection uses system/fallback font names only"], attributionRequired: false, distributed: false },
  ...
  const applied = applyWaivers([...packageSubjects, ...staticAssets, ...fixedSubjects], waivers, now);
  ...
  const overall = unresolved.length === 0 && applied.expiredWaivers.length === 0 ? "PASS" : "FAIL";
  return {
    schema: "website-design-compiler/repository-rights-clearance/v2",
    overall,
    ...
    counts,
    unresolved,
    expiredWaivers: applied.expiredWaivers,
