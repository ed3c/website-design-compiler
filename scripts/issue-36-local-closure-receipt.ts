...
async function main(){
  ...
  if(!safePath(commandPath)||!(resolve(root,commandPath).startsWith(`${root}${sep}`))){failures.push("command-results:path");}else try{commandBytes=await readFile(join(root,commandPath));commandValue=JSON.parse(commandBytes.toString("utf8"));}catch{failures.push("command-results:absent-or-invalid");}
  const command=evaluateIssue36CommandResults(commandValue,subject);failures.push(...command.failures);let commandEvidencePass=true;for(const item of command.commands.flatMap((entry)=>entry.evidence))try{if(sha256(await readFile(join(root,item.path)))!==item.sha256)throw new Error("digest");}catch{commandEvidencePass=false;failures.push(`command-evidence:${item.path}`);}
