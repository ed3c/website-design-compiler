...
function changedFiles(): string[] {
  try {
    return execFileSync("git", ["diff-tree", "--no-commit-id", "--name-only", "-r", process.env.GITHUB_SHA ?? "HEAD"], {
      cwd: root,
      encoding: "utf8"
...
const sharedBinding = await readJson<{
...
const project = await readJson<{
  referenceIntelligence?: Record<string, unknown>;
  graphics3d?: Record<string, unknown>;
  licenseProvenance?: Record<string, unknown>;
}>(projectPath);
...
const unresolvedRisks = [
  {
    id: "reference-remote-capture",
    state: project?.referenceIntelligence?.remoteCapture ?? "ABSENT",
    reason: "network-enabled URL capture is not exercised by the deterministic release fixture"
  },
  {
    id: "reference-image-capture",
    state: project?.referenceIntelligence?.imageCapture ?? "ABSENT",
    reason: "image-reference observation is not exercised by the deterministic release fixture"
  },
  {
    id: "reference-video-capture",
    state: project?.referenceIntelligence?.videoCapture ?? "ABSENT",
    reason: "video-reference observation is not exercised by the deterministic release fixture"
  },
  {
    id: "webgpu-tsl",
    state: project?.graphics3d?.webgpuTsl ?? "ABSENT",
    reason: "WebGPU/TSL remains an optional unexercised graphics path"
  },
  {
...
const commands = [
  ...
  "pnpm build",
  "pnpm test",
  "pnpm provenance:fixture",
  "pnpm compile:fixture",
  "pnpm ui:typecheck",
  "pnpm ui:build",
...
const receipt = {
  ...
  git: {
    ...
    event: process.env.GITHUB_EVENT_NAME ?? "UNBOUND",
    changedFiles: changedFiles()
  },
  workflow: {
    name: process.env.GITHUB_WORKFLOW ?? "UNBOUND",
    runId: process.env.GITHUB_RUN_ID ?? "UNBOUND",
    runAttempt: process.env.GITHUB_RUN_ATTEMPT ?? "UNBOUND",
    job: process.env.GITHUB_JOB ?? "UNBOUND",
    commands
  },
  environment: {
    ...
    osRelease: release(),
    arch: arch(),
    runnerOs: process.env.RUNNER_OS ?? "UNBOUND",
    runnerArch: process.env.RUNNER_ARCH ?? "UNBOUND",
    imageOs: process.env.ImageOS ?? "UNBOUND",
    imageVersion: process.env.ImageVersion ?? "UNBOUND"
  },
  bindings: sharedBinding ? {
    sourceRepository: sharedBinding.sourceRepository ?? "ABSENT",
    sourceIdentity: sharedBinding.sourceIdentity ?? "ABSENT",
    consumerIdentity: sharedBinding.consumerIdentity ?? "ABSENT",
    resolutions: sharedBinding.resolutions ?? []
  } : "ABSENT",
