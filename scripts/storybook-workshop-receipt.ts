...
const reviewedSourceRoots=["apps/site/app","apps/site/components","apps/site/.storybook","fixtures/showcase","src/semantic-design-tokens.ts","tests/storybook","playwright.storybook.config.ts","tsconfig.storybook.json"];
...
if(goldenAdmission?.state==="PASS"){
  ...
  try{
    ...
    await execFileAsync("git",["diff","--quiet",candidateSha,"HEAD","--",...reviewedSourceRoots],{cwd:process.cwd()});
  }catch(error){trustDiagnostics.push(`reviewed Storybook source does not cover the current UI subject: ${error instanceof Error?error.message:String(error)}`);}
