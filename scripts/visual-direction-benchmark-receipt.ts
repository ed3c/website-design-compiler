...
async function writeReceipt(overall: "PASS" | "FAIL", diagnostics: string[]): Promise<void> {
  ...
  console.log(JSON.stringify({ overall, categoryCount: categories.length, winnerDiversity, diagnostics }));
}
...
try {
  ...
  const message = error instanceof Error ? error.message : "visual-direction benchmark failed";
  const safeMessage = message.replaceAll(process.cwd(), "[workspace]");
  await writeReceipt("FAIL", [safeMessage]);
  process.exitCode = 1;
}
