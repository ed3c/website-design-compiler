...
const commands:Issue36Command[]=[];let stopped=false;for(const command of ISSUE_36_OWNING_COMMANDS){if(stopped){commands.push({command,verdict:"NOT_EXERCISED",exitCode:null,evidence:[]});continue;}const result=spawnSync("pnpm",[command.slice(5)],{cwd:root,env:environment,stdio:"inherit"});const exitCode=result.status??1;const evidence=[];const evidencePath=ISSUE_36_COMMAND_EVIDENCE[command];if(evidencePath)try{evidence.push({path:evidencePath,sha256:digest(await readFile(join(root,evidencePath)))});}catch{}const verdict=exitCode===0?"PASS" as const:"FAIL" as const;commands.push({command,verdict,exitCode,evidence});if(verdict==="FAIL")stopped=true;}
