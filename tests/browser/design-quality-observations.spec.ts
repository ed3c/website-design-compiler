...
for(const category of categories)test(`${category} emits browser-derived visual quality observations`,async({page},testInfo)=>{
  ...
  const computed=await root.evaluate((element)=>{
    ...
    const parseColor=(value:string):[number,number,number]|null=>{
      const match=/^rgba?\(\s*([0-9.]+)[, ]+\s*([0-9.]+)[, ]+\s*([0-9.]+)/.exec(value);
      if(!match)return null;
      return[Number(match[1]),Number(match[2]),Number(match[3])];
    };
    const luminance=(color:[number,number,number])=>{const channels=color.map((value)=>{const normalized=value/255;return normalized<=.04045?normalized/12.92:((normalized+.055)/1.055)**2.4;});return .2126*channels[0]!+.7152*channels[1]!+.0722*channels[2]!;};
    const contrast=(foreground:[number,number,number],background:[number,number,number])=>{const first=luminance(foreground);const second=luminance(background);return (Math.max(first,second)+.05)/(Math.min(first,second)+.05);};
    const textContrast=[...element.querySelectorAll<HTMLElement>("h1,h2,h3,p,a,button,li,blockquote,summary,td,th")].map((entry)=>{const foreground=parseColor(getComputedStyle(entry).color);let current:HTMLElement|null=entry;let background:[number,number,number]|null=null;while(current&&!background){const backgroundValue=getComputedStyle(current).backgroundColor;const parsed=parseColor(backgroundValue);const alpha=backgroundValue.match(/rgba?\\([^)]*[, ]\\s*([0-9.]+)\\s*\\)$/)?.[1];if(parsed&&(alpha===undefined||Number(alpha)>0))background=parsed;current=current.parentElement;}return foreground&&background?contrast(foreground,background):null;}).filter((value):value is number=>value!==null);
